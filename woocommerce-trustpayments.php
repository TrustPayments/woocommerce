    ...
    final class WooCommerce_TrustPayments {
		...
		public function after_remove_product_from_cart($removed_cart_item_key, $cart) {
			...
			if( $this->is_product_type_subscription($product) ) {
				...
				$serviceTransaction = WC_TrustPayments_Service_Transaction::instance();
		...
		public function register_order_statuses(){
			register_post_status('wc-trustp-redirected',
					array(
						'label' => 'Processing',
						'public' => true,
			...
			register_post_status('wc-trustp-waiting',
					array(
						'label' => 'Waiting',
						'public' => true,
						'exclude_from_search' => false,
			...
			register_post_status('wc-trustp-manual',
					array(
		...
		public function set_device_id_cookie(){
			...
			if (isset($_COOKIE['wc_trustpayments_device_id']) && !empty($_COOKIE['wc_trustpayments_device_id'])) {
				$value = sanitize_text_field($_COOKIE['wc_trustpayments_device_id']);
			}
		...
		public function enqueue_javascript_script(){
			if(is_cart() || is_checkout()){
				$unique_id = sanitize_text_field($_COOKIE['wc_trustpayments_device_id']);
				...
				$script_url = WC_TrustPayments_Helper::instance()->get_base_gateway_url() . 's/' . 
						...
						$unique_id;
				wp_enqueue_script('trustpayments-device-id-js', $script_url, array(), null, true);
			}
		}
		
		public function enqueue_stylesheets(){
		    if(is_checkout()){
		        wp_enqueue_style( 'trustpayments-checkout-css', $this->plugin_url() . '/assets/css/checkout.css' );
		    }
		}
		...
		public function add_gateways($methods){
		    $space_id = get_option(WooCommerce_TrustPayments::CK_SPACE_ID);
			$method_configurations = WC_TrustPayments_Entity_Method_Configuration::load_by_states_and_space_id($space_id,
					array(
		...
		public function woocommerce_attribute_added($attribute_id, $data){
		    if(did_action('product_page_product_attributes')){
		        ...
		        $send = isset( $_POST['trustpayments_attribute_option_send'] ) ? 1 : 0;
