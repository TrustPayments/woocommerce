...
/**
 *
 * WC_TrustPayments_Unique_Id Class
 *
 * TrustPayments
 ...
 */
/**
 * This class handles the webhooks of TrustPayments
 */
class WC_TrustPayments_Webhook_Handler {
	...
	public static function init() {
		add_action(
			'woocommerce_api_trustpayments_webhook',
			array(
				__CLASS__,
				'process',
			)
	...
	public static function process() {
		$webhook_service = WC_TrustPayments_Service_Webhook::instance();
		...
		try {
			$request = new WC_TrustPayments_Webhook_Request( json_decode( $request_body ) );
			$webhook_model = $webhook_service->get_webhook_entity_for_id( $request->get_listener_entity_id() );
			if ( null === $webhook_model ) {
				WooCommerce_TrustPayments::instance()->log( sprintf( 'Could not retrieve webhook model for listener entity id: %s', $request->get_listener_entity_id() ), WC_Log_Levels::ERROR );
			    	// phpcs:ignore
...
WC_TrustPayments_Webhook_Handler::init();
