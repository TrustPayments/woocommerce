 ...
 */
/**
 * This class handles the email settings fo Trust Payments.
 */
class WC_TrustPayments_Email {
	...
	public static function send_email_for_order($enabled, $order){
		...
		if (isset($GLOBALS['_trustpayments_resend_email']) && $GLOBALS['_trustpayments_resend_email']) {
			return $enabled;
		}
		$gateway = wc_get_payment_gateway_by_order($order);
		if ($gateway instanceof WC_TrustPayments_Gateway) {
		    $send = get_option(WooCommerce_TrustPayments::CK_SHOP_EMAIL, "yes");
		    if ($send != "yes") {
		        return false;
		    }
		}		
	...
	public static function before_resend_email($order){
		$GLOBALS['_trustpayments_resend_email'] = true;
	}

	public static function after_resend_email($order, $email){
		unset($GLOBALS['_trustpayments_resend_email']);
	}
	
	public static function add_email_actions($actions){
	    
		$to_add = array(
			'woocommerce_order_status_trustp-redirected_to_processing',
			'woocommerce_order_status_trustp-redirected_to_completed',
			'woocommerce_order_status_trustp-redirected_to_on-hold',
			'woocommerce_order_status_trustp-redirected_to_trustpayments-waiting',
			'woocommerce_order_status_trustp-redirected_to_trustpayments-manual',
			'woocommerce_order_status_trustp-manual_to_cancelled',
			'woocommerce_order_status_trustp-waiting_to_cancelled',
			'woocommerce_order_status_trustp-manual_to_processing',
			'woocommerce_order_status_trustp-waiting_to_processing'
		);
		...
		if(class_exists('woocommerce_wpml')){
		    ...
		    if(!is_null($woocommerce_wpml)){
    			...
    			$notifications_all =  array(
    				'woocommerce_order_status_trustp-redirected_to_processing_notification',
    				'woocommerce_order_status_trustp-redirected_to_completed_notification',
    				'woocommerce_order_status_trustp-redirected_to_on-hold_notification',
    				'woocommerce_order_status_trustp-redirected_to_trustpayments-waiting_notification',
    				'woocommerce_order_status_trustp-redirected_to_trustpayments-manual_notification',
    			);
    			$notifications_customer = array(
    				'woocommerce_order_status_trustp-manual_to_processing_notification',
    				'woocommerce_order_status_trustp-waiting_to_processing_notification',
    				'woocommerce_order_status_on-hold_to_processing_notification',
    				'woocommerce_order_status_trustp-manual_to_cancelled_notification',
    				'woocommerce_order_status_trustp-waiting_to_cancelled_notifcation'
    			);
	...
	public static function add_email_classes($emails){
	    ...
	    if(isset($emails['WC_GZD_Email_Customer_Paid_For_Order'])){
	        ...
	        add_action( 'woocommerce_order_status_trustp-redirected_to_processing_notification', array( $emailObject, 'trigger' ), 10, 2 );
	        add_action( 'woocommerce_order_status_trustp-manual_to_processing_notification', array( $emailObject, 'trigger' ), 10, 2 );
	        add_action( 'woocommerce_order_status_trustp-waiting_to_processing_notification', array( $emailObject, 'trigger' ), 10, 2 );
		...
		foreach($emails as $key => $emailObject){
			switch($key){
				case 'WC_Email_New_Order':
					add_action( 'woocommerce_order_status_trustp-redirected_to_processing_notification', array( $emailObject, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_completed_notification', array( $emailObject, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_on-hold_notification', array( $emailObject, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_trustpayments-waiting_notification', array( $emailObject, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_trustpayments-manual_notification', array( $emailObject, 'trigger' ), 10, 2 );
					
					break;
					
				case 'WC_Email_Cancelled_Order':
					add_action( 'woocommerce_order_status_trustp-manual_to_cancelled_notification', array( $emailObject, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-waiting_to_cancelled_notification', array( $emailObject, 'trigger' ), 10, 2 );
					break;
