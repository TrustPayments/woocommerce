...
class WC_TrustPayments_Email {
	...
	public static function add_email_classes( $emails ) {
		...
		if ( isset( $emails['WC_GZD_Email_Customer_Paid_For_Order'] ) ) {
			...
			add_action( 'woocommerce_order_status_trustp-redirected_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
			add_action( 'woocommerce_order_status_trustp-manual_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
			add_action( 'woocommerce_order_status_trustp-waiting_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
		...
		foreach ( $emails as $key => $email_object ) {
			switch ( $key ) {
				case 'WC_Email_New_Order':
					add_action( 'woocommerce_order_status_trustp-redirected_to_processing_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_completed_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_on-hold_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_trustpayments-waiting_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-redirected_to_trustpayments-manual_notification', array( $email_object, 'trigger' ), 10, 2 );

					break;

				case 'WC_Email_Cancelled_Order':
					add_action( 'woocommerce_order_status_trustp-manual_to_cancelled_notification', array( $email_object, 'trigger' ), 10, 2 );
					add_action( 'woocommerce_order_status_trustp-waiting_to_cancelled_notification', array( $email_object, 'trigger' ), 10, 2 );
					break;
