...
/**
 *
 * WC_TrustPayments_Admin_Refund Class
 *
 * TrustPayments
...
class WC_TrustPayments_Admin_Refund {
	...
	public static function render_refund_button_state( WC_Order $order ) {
		...
		if ( $gateway instanceof WC_TrustPayments_Gateway ) {
			...
			if ( ! in_array( $transaction_info->get_state(), self::$refundable_states ) ) {
				echo '<span id="trustpayments-remove-refund" style="display:none;"></span>';
			} else {
				$existing_refund_job = WC_TrustPayments_Entity_Refund_Job::load_running_refund_for_transaction(
					$transaction_info->get_space_id(),
					$transaction_info->get_transaction_id()
				);
				if ( $existing_refund_job->get_id() > 0 ) {
					echo '<span class="trustpayments-action-in-progress">' . esc_html( __( 'There is a refund in progress.', 'woo-trustpayments' ) ) . '</span>';
					echo '<button type="button" class="button trustpayments-update-order">' . esc_html( __( 'Update', 'woo-trustpayments' ) ) . '</button>';
					echo '<span id="trustpayments-remove-refund" style="display:none;"></span>';
				}
	...
	public static function render_refund_states( $order_id ) {
		...
		if ( ! empty( $refunds ) ) {
			...
			foreach ( $refunds as $refund ) {
				echo '<div class="trustpayments-refund-status" data-refund-id="' . esc_attr( $refund->get_wc_refund_id() ) . '" data-refund-state="' .
	...
	public static function execute_refund( WC_Order $order, WC_Order_Refund $refund ) {
		...
		try {
			...
			if ( WC_TrustPayments_Entity_Refund_Job::count_running_refund_for_transaction(
				$transaction_info->get_space_id(),
				$transaction_info->get_transaction_id()
			) > 0 ) {
	...
	public static function update_refunds() {
		...
		foreach ( $to_process as $id ) {
			try {
				...
				$message = sprintf( __( 'Error updating refund job with id %1$d: %2$s', 'woo-trustpayments' ), $id, $e->getMessage() );
	 ...
	 * @param WC_Order                                      $order wc_order.
	 * @param WC_Order_Refund                               $refund refund.
	 * @param \TrustPayments\Sdk\Model\RefundCreate $refund_create refund_create.
