...
/**
 *
 * WC_TrustPayments_Admin_Settings_Page Class
 *
 * TrustPayments
...
/**
 * Class WC_TrustPayments_Admin_Settings_Page.
 *
 * @class WC_TrustPayments_Admin_Settings_Page
 */
/**
...
class WC_TrustPayments_Admin_Settings_Page extends WC_Settings_Page {
	...
	public function __construct() {
		...
		add_action(
			'woocommerce_admin_field_trustpayments_links',
			array(
				$this,
				'output_links',
			)
	...
	public function update_settings() {
		...
		if ( ! ( empty( $user_id ) || empty( $user_key ) ) ) {
			...
			try {
				...
				$error_message = __( 'Could not update payment method configuration.', 'woo-trustpayments' );
			...
			try {
				...
				WooCommerce_TrustPayments::instance()->log( $e->getMessage(), WC_Log_Levels::ERROR );
				WooCommerce_TrustPayments::instance()->log( $e->getTraceAsString(), WC_Log_Levels::DEBUG );
				$error_message = __( 'Could not install webhooks, please check if the feature is active in your space.', 'woo-trustpayments' );
				WC_Admin_Settings::add_error( $error_message );
			}
			try {
				...
				WooCommerce_TrustPayments::instance()->log( $e->getMessage(), WC_Log_Levels::ERROR );
				WooCommerce_TrustPayments::instance()->log( $e->getTraceAsString(), WC_Log_Levels::DEBUG );
				$error_message = __( 'Could not update the manual task list.', 'woo-trustpayments' );
				WC_Admin_Settings::add_error( $error_message );
			}
			try {
			...
			if ( ! empty( $error_message ) ) {
				$error_message = __( 'Please check your credentials and grant the application user the necessary rights (Account Admin) for your space.', 'woo-trustpayments' );
				WC_Admin_Settings::add_error( $error_message );
			}
	...
	public function get_settings() {
		...
		$settings = array(
			array(
				'links' => array(
					'https://plugin-documentation.ep.trustpayments.com/TrustPayments/woocommerce/2.0.0-alpha/docs/en/documentation.html' => __( 'Documentation', 'woo-trustpayments' ),
