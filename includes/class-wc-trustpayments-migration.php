...
/**
 *
 * WC_TrustPayments_Migration Class
 *
 * TrustPayments
...
class WC_TrustPayments_Migration {
	...
	private static function check_requirements() {
		...
		if ( version_compare( PHP_VERSION, WC_TRUSTPAYMENTS_REQUIRED_PHP_VERSION, '<' ) ) {
			$errors[] = sprintf( __( "PHP %1\$s+ is required. (You're running version %2\$s)", 'woo-trustpayments' ), WC_TRUSTPAYMENTS_REQUIRED_PHP_VERSION, PHP_VERSION );
		}
		if ( version_compare( $wp_version, WC_TRUSTPAYMENTS_REQUIRED_WP_VERSION, '<' ) ) {
			$errors[] = sprintf( __( "Wordpress %1\$s+ is required. (You're running version %2\$s)", 'woo-trustpayments' ), WC_TRUSTPAYMENTS_REQUIRED_WP_VERSION, $wp_version );
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( WC_TRUSTPAYMENTS_PLUGIN_BASENAME === $file ) {
			$row_meta = array(
				'docs' => '<a href="https://plugin-documentation.ep.trustpayments.com/TrustPayments/woocommerce/2.1.17/docs/en/documentation.html" aria-label="' . esc_attr__( 'View Documentation', 'woo-trustpayments' ) . '">' . esc_html__( 'Documentation', 'woo-trustpayments' ) . '</a>',
			);
	...
	public static function supported_payments_integration_notice() {
		...
		if (version_compare( $woocommerce_data['Version'], WC_TRUSTPAYMENTS_REQUIRED_WC_MAXIMUM_VERSION, '>' )) {
		    ...
		    $message = __( 'A version of the TrustPayments plugin is yet to be released for this version of WooCommerce.', 'sample-text-domain' );
