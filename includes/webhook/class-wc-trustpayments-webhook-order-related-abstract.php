...
/**
 *
 * WC_TrustPayments_Webhook_Order_Related_Abstract Class
 *
 * TrustPayments
 ...
 */
abstract class WC_TrustPayments_Webhook_Order_Related_Abstract extends WC_TrustPayments_Webhook_Abstract {
	...
	public function process( WC_TrustPayments_Webhook_Request $request ) {
		...
		try {
			WC_TrustPayments_Helper::instance()->lock_by_transaction_id( $request->get_space_id(), $this->get_transaction_id( $entity ) );
	 ...
	 * @return object
	 */
	abstract protected function load_entity( WC_TrustPayments_Webhook_Request $request);
