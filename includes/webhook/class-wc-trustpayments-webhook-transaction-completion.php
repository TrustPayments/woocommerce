...
/**
 *
 * WC_TrustPayments_Webhook_Transaction_Completion Class
 *
 * TrustPayments
 ...
 */
class WC_TrustPayments_Webhook_Transaction_Completion extends WC_TrustPayments_Webhook_Order_Related_Abstract {
	 ...
	 * @return object|\TrustPayments\Sdk\Model\TransactionCompletion TransactionCompletion.
	...
	protected function get_order_id( $completion ) {
		...
		return WC_TrustPayments_Entity_Transaction_Info::load_by_transaction( $completion->getLineItemVersion()->getTransaction()->getLinkedSpaceId(), $completion->getLineItemVersion()->getTransaction()->getId() )->get_order_id();
	}
	 ...
	 * Success.
	 *
	 * @param \TrustPayments\Sdk\Model\TransactionCompletion $completion completion.
	 * @param WC_Order                                               $order order.
	 * @return void
	 * @throws Exception Exception.
	 */
	protected function success( \TrustPayments\Sdk\Model\TransactionCompletion $completion, WC_Order $order ) {
		...
		if ( ! $completion_job->get_id() ) {
			...
			$completion_job = WC_TrustPayments_Entity_Completion_Job::load_running_completion_for_transaction(
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( $changed_qty > 0 ) {
						...
						$order->add_order_note(
							...
							sprintf( __( '%1$s stock increased from %2$s to %3$s.', 'woo-trustpayments' ), $item_name, $old_stock, $new_stock )
						);
						do_action( 'wc_trustpayments_restock_not_completed_item', $product->get_id(), $old_stock, $new_stock, $order, $product );
					}
				}
			}
		}
