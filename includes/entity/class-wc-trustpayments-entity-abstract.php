...
/**
 *
 * WC_TrustPayments_Entity_Abstract Class
 *
 * TrustPayments
...
/**
 * Class WC_TrustPayments_Entity_Abstract.
 *
 * @class WC_TrustPayments_Entity_Abstract
 */
/**
 ...
 */
abstract class WC_TrustPayments_Entity_Abstract {
	/**
	 * Data
	 *
	 * @var $data data.
	...
	protected static function get_base_fields() {
		return array(
			...
			'created_at' => WC_TrustPayments_Entity_Resource_Type::DATETIME,
			'updated_at' => WC_TrustPayments_Entity_Resource_Type::DATETIME,
		);
	}
	...
	protected function fill_values_from_db( array $db_values ) {
		...
		foreach ( $fields as $key => $type ) {
			if ( isset( $db_values[ $key ] ) ) {
				$value = $db_values[ $key ];
				switch ( $type ) {
					case WC_TrustPayments_Entity_Resource_Type::STRING:
						// Do nothing.
						break;
					case WC_TrustPayments_Entity_Resource_Type::BOOLEAN:
						$value = 'Y' === $value;
						break;
					case WC_TrustPayments_Entity_Resource_Type::INTEGER:
						$value = intval( $value );
						break;

					case WC_TrustPayments_Entity_Resource_Type::DECIMAL:
						$value = (float) $value;
						break;

					case WC_TrustPayments_Entity_Resource_Type::DATETIME:
						$value = new DateTime( $value );
						break;

					case WC_TrustPayments_Entity_Resource_Type::OBJECT:
