...
class WC_TrustPayments_Helper {
	 ...
	 * Instance.
	 *
	 * @return WC_TrustPayments_Helper
	 */
	...
	public function delete_provider_transients() {
		$transients = array(
			'wc_trustpayments_currencies',
			'wc_trustpayments_label_description_groups',
			'wc_trustpayments_label_descriptions',
			'wc_trustpayments_languages',
			'wc_trustpayments_payment_connectors',
			'wc_trustpayments_payment_methods',
		);
	...
	public function get_api_client() {
		if ( null === $this->api_client ) {
			...
			if ( ! empty( $user_id ) && ! empty( $user_key ) ) {
				$this->api_client = new \TrustPayments\Sdk\ApiClient( $user_id, $user_key );
				...
				throw new Exception( __( 'The API access data is incomplete.', 'woo-trustpayments' ) );
			}
		}
	...
	public function get_base_gateway_url() {
		return get_option( 'wc_trustpayments_base_gateway_url', 'https://ep.trustpayments.com/' );
	}
	...
	public function translate( $translated_string, $language = null ) {
		...
		try {
			/* @var WC_TrustPayments_Provider_Language $language_provider */
			$language_provider = WC_TrustPayments_Provider_Language::instance();
	...
	/**
	 * Returns the URL to a resource on TrustPayments in the given context (space, space view, language).
	 *
	...
	public function get_currency_fraction_digits( $currency_code ) {
		/* @var WC_TrustPayments_Provider_Currency $currency_provider */
		$currency_provider = WC_TrustPayments_Provider_Currency::instance();
	...
	public function get_total_amount_including_tax( array $line_items, bool $excludeDiscounts = false) {
		$sum = 0;
		/** @var \TrustPayments\Sdk\Model\LineItemCreate $line_item */
		foreach ( $line_items as $line_item ) {
			$type = $line_item->getType();
			$name = $line_item->getName();

			if ( $excludeDiscounts && $type === \TrustPayments\Sdk\Model\LineItemType::DISCOUNT && strpos( $name, WC_TrustPayments_Packages_Coupon_Discount::COUPON ) !== false ) {
	...
	public function cleanup_line_items( array $line_items, $expected_sum, $currency ) {
		...
		$has_coupons = apply_filters( 'wc_trustpayments_packages_coupon_cart_has_coupon_discounts_applied', $currency );
		...
		if ( 0 != $inconsistent_amount ) {
			$enforce_consistency = get_option( WooCommerce_TrustPayments::CK_ENFORCE_CONSISTENCY );
			switch ( $enforce_consistency ) {
				case 'no':
					...
					$line_item->setName( __( 'Adjustment', 'woo-trustpayments' ) );
					$line_item->setQuantity( 1 );
					$line_item->setSku( 'adjustment' );
					$line_item->setUniqueId( 'adjustment' );
					$line_item->setShippingRequired( false );
					$line_item->setType( $enforce_consistency > 0 ? \TrustPayments\Sdk\Model\LineItemType::FEE : \TrustPayments\Sdk\Model\LineItemType::DISCOUNT );
					$line_items[] = $line_item;
					break;
				default:
					throw new WC_TrustPayments_Exception_Invalid_Transaction_Amount( $effective_sum, $rounded_expected_sum );
			}
		}
	...
	public function get_current_cart_id() {
		...
		$current_cart_id = $session_handler->get( 'trustpayments_current_cart_id', null );
		if ( null === $current_cart_id ) {
			$current_cart_id = WC_TrustPayments_Unique_Id::get_uuid();
			$session_handler->set( 'trustpayments_current_cart_id', $current_cart_id );
		}
	...
	public function destroy_current_cart_id() {
		$session_handler = WC()->session;
		$session_handler->set( 'trustpayments_current_cart_id', null );
	}
	...
	public function lock_by_transaction_id( $space_id, $transaction_id ) {
		...
		$wpdb->query( $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'wc_trustpayments_transaction_info WHERE transaction_id = %d and space_id = %d FOR UPDATE', $transaction_id, $space_id ) );

		$wpdb->update(
			$wpdb->prefix . 'wc_trustpayments_transaction_info',
			$data_array,
			array(
	...
	public function get_clean_locale_for_string( $language_string, $use_default ) {
		...
		if ( strlen( $language_string ) >= 5 ) {
			...
			$language = WC_TrustPayments_Provider_Language::instance()->find( $language_string );
			if ( ! $language && strpos( $language_string, '-' ) !== false ) {
				...
				while ( ! $language && ! empty( $language_parts ) ) {
					$language = WC_TrustPayments_Provider_Language::instance()->find( implode( '-', $language_parts ) );
					array_pop( $language_parts );
				}
			}
		}
		if ( ! $language ) {
			...
			$language = WC_TrustPayments_Provider_Language::instance()->find_by_iso_code( $language_string );
		}
	...
	public function try_to_parse_date( $date_string ) {
		$date_of_birth = false;
		$custom_date_of_birth_format = apply_filters( 'wc_trustpayments_custom_date_of_birth_format', '' );
	...
	protected function restock_items_for_order( WC_Order $order ) {
		if ( 'yes' === get_option( 'woocommerce_manage_stock' ) && $order && apply_filters( 'wc_trustpayments_can_increase_order_stock', true, $order ) && count( $order->get_items() ) > 0 ) {
			foreach ( $order->get_items() as $item ) {
				...
				if ( $item->is_type( 'line_item' ) && $product && $product->managing_stock() ) {
					...
					if ( ! is_wp_error( $new_stock ) ) {
						...
						$order->add_order_note( sprintf( __( '%1$s stock increased from %2$s to %3$s.', 'woo-trustpayments' ), $item_name, $new_stock - $qty, $new_stock ) );
					}
				}
			}
			do_action( 'wc_trustpayments_restocked_order', $order );
		}
	}
