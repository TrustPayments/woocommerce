...
class WC_TrustPayments_Helper
{
    ...
    public function lock_by_transaction_id($space_id, $transaction_id)
    {
        ...
        $wpdb->query($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "woocommerce_trustpayments_transaction_info WHERE transaction_id = %d and space_id = %d FOR UPDATE", $transaction_id, $space_id));
        
        $wpdb->update($wpdb->prefix . 'woocommerce_trustpayments_transaction_info', $data_array, array(
    ...
    {
        if ('yes' === get_option('woocommerce_manage_stock') && $order && apply_filters('wc_trustpayments_can_increase_order_stock', true, $order) && sizeof($order->get_items()) > 0) {
