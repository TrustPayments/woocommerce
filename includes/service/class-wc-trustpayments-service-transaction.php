...
/**
 *
 * WC_TrustPayments_Service_Transaction Class
 *
 * TrustPayments
 ...
 */
class WC_TrustPayments_Service_Transaction extends WC_TrustPayments_Service_Abstract {
	 ...
	 *
	 * @var \TrustPayments\Sdk\Service\TransactionLightboxService
	 */
	...
	protected function get_transaction_service() {
		if ( is_null( $this->transaction_service ) ) {
			$this->transaction_service = new \TrustPayments\Sdk\Service\TransactionService( WC_TrustPayments_Helper::instance()->get_api_client() );
		}
	...
	protected function get_transaction_iframe_service() {
		if ( is_null( $this->transaction_iframe_service ) ) {
			$this->transaction_iframe_service = new \TrustPayments\Sdk\Service\TransactionIframeService( WC_TrustPayments_Helper::instance()->get_api_client() );
		}
	...
	protected function get_transaction_lightbox_service() {
		if ( is_null( $this->transaction_lightbox_service ) ) {
			$this->transaction_lightbox_service = new \TrustPayments\Sdk\Service\TransactionLightboxService( WC_TrustPayments_Helper::instance()->get_api_client() );
		}
	...
	protected function get_transaction_payment_page_service() {
		if ( is_null( $this->transaction_payment_page_service ) ) {
			$this->transaction_payment_page_service = new \TrustPayments\Sdk\Service\TransactionPaymentPageService( WC_TrustPayments_Helper::instance()->get_api_client() );
		}
	...
	public function wait_for_transaction_state( WC_Order $order, array $states, $max_wait_time = 10 ) {
		...
		while ( true ) {
			...
			$transaction_info = WC_TrustPayments_Entity_Transaction_Info::load_newest_by_mapped_order_id( $order->get_id() );
	 ...
	 */
	public function get_javascript_url_for_transaction( \TrustPayments\Sdk\Model\Transaction $transaction ) {
	 ...
	 * @throws Exception Exception.
	 */
	public function get_lightbox_url_for_transaction( \TrustPayments\Sdk\Model\Transaction $transaction ) {
	...
	public function get_failed_charge_attempt( $space_id, $transaction_id ) {
		...
		$filter->setChildren(
			array(
				...
				$this->create_entity_filter( 'state', \TrustPayments\Sdk\Model\ChargeAttemptState::FAILED ),
			)
		);
	 ...
	 * @throws Exception Exception.
	 */
	public function update_transaction_info( \TrustPayments\Sdk\Model\Transaction $transaction, WC_Order $order ) {
		$info = WC_TrustPayments_Entity_Transaction_Info::load_by_transaction( $transaction->getLinkedSpaceId(), $transaction->getId() );
		...
		$info = apply_filters( 'wc_trustpayments_update_transaction_info', $info, $transaction, $order );
		$info->save();
		return $info;
	}
	 ...
	 * @throws Exception Exception.
	 */
	protected function get_transaction_labels( \TrustPayments\Sdk\Model\Transaction $transaction ) {
	 ...
	 * @throws Exception Exception.
	 */
	protected function get_charge_attempt( \TrustPayments\Sdk\Model\Transaction $transaction ) {
		$charge_attempt_service = $this->get_charge_attempt_service();
		$query = new \TrustPayments\Sdk\Model\EntityQuery();
		$filter = new \TrustPayments\Sdk\Model\EntityQueryFilter();
		$filter->setType( \TrustPayments\Sdk\Model\EntityQueryFilterType::_AND );
		$filter->setChildren(
			array(
				...
				$this->create_entity_filter( 'state', \TrustPayments\Sdk\Model\ChargeAttemptState::SUCCESSFUL ),
			)
		);
		$query->setFilter( $filter );
		$query->setNumberOfEntities( 1 );
	 ...
	 * @return string
	 */
	protected function get_payment_method_image( \TrustPayments\Sdk\Model\Transaction $transaction, WC_order $order ) {
		if ( is_null( $transaction->getPaymentConnectorConfiguration() ) ) {
			$method_instance = wc_get_payment_gateway_by_order( $order );
			if ( false != $method_instance && ( $method_instance instanceof WC_TrustPayments_Gateway ) ) {
				return WC_TrustPayments_Helper::instance()->get_resource_url( $method_instance->get_payment_method_configuration()->get_image_base(), $method_instance->get_payment_method_configuration()->get_image() );
			}
			return null;
		}
	 ...
	 *
	 * @return array|int[]|\TrustPayments\Sdk\Model\PaymentMethodConfiguration
	 * @throws WC_TrustPayments_Exception_Invalid_Transaction_Amount WC_TrustPayments_Exception_Invalid_Transaction_Amount.
	 * @throws \TrustPayments\Sdk\ApiException ApiException.
	 */
	public function get_possible_payment_methods_for_cart() {

		$current_cart_id = WC_TrustPayments_Helper::instance()->get_current_cart_id();
	...
	public function confirm_transaction( $transaction_id, $space_id, WC_Order $order, $method_configuration_id ) {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				if ( $transaction->getState() != \TrustPayments\Sdk\Model\TransactionState::PENDING ) {
					throw new Exception( __( 'The checkout expired, please try again.', 'woo-trustpayments' ) );
				}
				...
				$pending_transaction = apply_filters( 'wc_trustpayments_modify_confirm_transaction', $pending_transaction, $order );
				return $this->get_transaction_service()->confirm( $space_id, $pending_transaction );
			} catch ( \Exception $e ) {
				$last = $e;
			}
		}
		WooCommerce_TrustPayments::instance()->log( __CLASS__ . ' : ' . __FUNCTION__ . ' : ' . __LINE__ . ' : ' . $last->getMessage(), WC_Log_Levels::ERROR );
		throw $last;
	}
	 ...
	 * @throws WC_TrustPayments_Exception_Invalid_Transaction_Amount WC_TrustPayments_Exception_Invalid_Transaction_Amount.
	 */
	protected function assemble_order_transaction_data( WC_Order $order, \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction ) {
		...
		if ( ! empty( $language_string ) ) {
			$language = WC_TrustPayments_Helper::instance()->get_clean_locale_for_string( $language_string, false );
		}
		if ( empty( $language ) ) {
	...
	protected function getOrderReference( $order ) {
		$reference_type = get_option( WooCommerce_TrustPayments::CK_ORDER_REFERENCE );

		if ( WC_TrustPayments_Order_Reference::ORDER_NUMBER == $reference_type ) {
	 ...
	 * @param \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction transaction.
	 */
	protected function set_order_return_urls( WC_Order $order, \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction ) {
	...
	protected function set_order_line_items( WC_Order $order, \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction ) {
		$transaction->setLineItems( WC_TrustPayments_Service_Line_Item::instance()->get_items_from_order( $order ) );
	}
	...
	protected function get_order_billing_address( WC_order $order ) {
		...
		$custom_billing_date_of_birth_meta_name = apply_filters( 'wc_trustpayments_billing_date_of_birth_order_meta_name', '' );
		...
		if ( ! empty( $date_of_birth_string ) ) {
			$date_of_birth = WC_TrustPayments_Helper::instance()->try_to_parse_date( $date_of_birth_string );
		...
		$gender_string = '';
		$custom_billing_gender_meta_name = apply_filters( 'wc_trustpayments_billing_gender_order_meta_name', '' );
		...
		if ( ! empty( $gender_string ) ) {
			if ( strtolower( $gender_string ) == 'm' || strtolower( $gender_string ) == 'male' ) {
				$address->setGender( \TrustPayments\Sdk\Model\Gender::MALE );
			} elseif ( strtolower( $gender_string ) == 'f' || strtolower( $gender_string ) == 'female' ) {
				$address->setGender( \TrustPayments\Sdk\Model\Gender::FEMALE );
			}
		}

		return apply_filters( 'wc_trustpayments_modify_order_billing_address', $address, $order );
	}
	...
	protected function get_order_shipping_address( WC_order $order ) {
		...
		$date_of_birth_string = '';
		$custom_shipping_date_of_birth_meta_name = apply_filters( 'wc_trustpayments_shipping_date_of_birth_order_meta_name', '' );
		...
		$gender_string = '';
		$custom_shipping_gender_meta_name = apply_filters( 'wc_trustpayments_shipping_gender_order_meta_name', '' );
		...
		return apply_filters( 'wc_trustpayments_modify_order_shipping_address', $address, $order );
	...
	public function get_transaction_from_session() {
		...
		if ( ! isset( self::$transaction_cache[ $current_cart_id ] ) || null == self::$transaction_cache[ $current_cart_id ] ) {
			$transaction_id = WC()->session->get( 'trustpayments_transaction_id', null );
			$space_id = WC()->session->get( 'trustpayments_space_id', null );
			$configured_space_id = get_option( WooCommerce_TrustPayments::CK_SPACE_ID );
	...
	public function get_transaction_from_order( WC_Order $order ) {
		...
		if ( ! isset( self::$transaction_cache[ $order->get_id() ] ) || is_null( self::$transaction_cache[ $order->get_id() ] ) ) {
			...
			} elseif ( $existing_transaction->get_state() == \TrustPayments\Sdk\Model\TransactionState::PENDING ) {
	...
	protected function create_transaction_by_order( WC_Order $order ) {
		...
		$space_view_id = get_option( WooCommerce_TrustPayments::CK_SPACE_VIEW_ID, null );
		if ( ! empty( $space_view_id ) ) {
			$create_transaction->setSpaceViewId( $space_view_id );
		}
		$create_transaction->setAutoConfirmationEnabled( false );
		if ( isset( $_COOKIE['wc_trustpayments_device_id'] ) ) {
			$create_transaction->setDeviceSessionIdentifier( sanitize_text_field( wp_unslash( $_COOKIE['wc_trustpayments_device_id'] ) ) );
		}
		$this->assemble_order_transaction_data( $order, $create_transaction );
		$create_transaction = apply_filters( 'wc_trustpayments_modify_order_create_transaction', $create_transaction, $order );
	...
	protected function create_transaction_from_session() {
		...
		$create_transaction = apply_filters( 'wc_trustpayments_modify_session_create_transaction', $create_transaction );
		$transaction = $this->get_transaction_service()->create( $space_id, $create_transaction );
	 ...
	 *
	 * @param WC_Order                                         $order order.
	 * @param WC_TrustPayments_Entity_Transaction_Info $existing_transaction existing transaction.
	 * @return \TrustPayments\Sdk\Model\Transaction|\TrustPayments\Sdk\Model\TransactionCreate
	 * @throws Exception Exception.
	 */
	protected function load_and_update_transaction_for_order( WC_Order $order, WC_TrustPayments_Entity_Transaction_Info $existing_transaction ) {
		$last = new \Exception( __FUNCTION__ );
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				$pending_transaction->setVersion( $transaction->getVersion() );
				$this->assemble_order_transaction_data( $order, $pending_transaction );
				$pending_transaction = apply_filters( 'wc_trustpayments_modify_order_pending_transaction', $pending_transaction, $order );
	...
	protected function load_and_update_transaction_from_session() {
		...
		for ( $i = 0; $i < 5; $i++ ) {
			try {
				...
				$space_id = $session_handler->get( 'trustpayments_space_id' );
				$transaction_id = $session_handler->get( 'trustpayments_transaction_id' );
				$transaction = $this->get_transaction( $space_id, $transaction_id );
				if ( $transaction->getState() != \TrustPayments\Sdk\Model\TransactionState::PENDING || ( ! empty( $transaction->getCustomerId() ) && $transaction->getCustomerId() != $this->get_customer_id() ) ) {
				...
				$pending_transaction = apply_filters( 'wc_trustpayments_modify_session_pending_transaction', $pending_transaction );
				return $this->get_transaction_service()->update( $space_id, $pending_transaction );
			} catch ( \Exception $e ) {
				$last = $e;
			}
	 ...
	 * @param \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction transaction.
	 */
	protected function assemble_session_transaction_data( \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction ) {
		...
		$transaction->setCustomerId( $this->get_customer_id() );
		$transaction->setLanguage( WC_TrustPayments_Helper::instance()->get_cleaned_locale() );
	 ...
	 * @throws WC_TrustPayments_Exception_Invalid_Transaction_Amount WC_TrustPayments_Exception_Invalid_Transaction_Amount.
	 */
	protected function set_session_line_items( \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction ) {
		$transaction->setLineItems( WC_TrustPayments_Service_Line_Item::instance()->get_items_from_session() );
	}

	/**
	 * Returns the billing address of the current session.
	 *
	 * @return \TrustPayments\Sdk\Model\AddressCreate|null
	 */
	protected function get_session_billing_address() {
		...
		$date_of_birth_string = $customer->get_meta( '_trustpayments_billing_date_of_birth', true, 'edit' );
		if ( ! empty( $date_of_birth_string ) ) {
			$date_of_birth = WC_TrustPayments_Helper::instance()->try_to_parse_date( $date_of_birth_string );
			if ( false !== $date_of_birth ) {
				$address->setDateOfBirth( $date_of_birth );
		...
		$gender_string = $customer->get_meta( '_trustpayments_billing_gender', true, 'edit' );
		if ( ! empty( $gender_string ) ) {
			if ( strtolower( $gender_string ) == 'm' || strtolower( $gender_string ) == 'male' ) {
				$address->setGender( \TrustPayments\Sdk\Model\Gender::MALE );
			} elseif ( strtolower( $gender_string ) == 'f' || strtolower( $gender_string ) == 'female' ) {
		...
		return apply_filters( 'wc_trustpayments_modify_session_billing_address', $address );
	}
	...
	protected function get_session_shipping_address() {
		...
		$date_of_birth_string = $customer->get_meta( '_trustpayments_shipping_date_of_birth', true, 'edit' );
		if ( ! empty( $date_of_birth_string ) ) {
			$date_of_birth = WC_TrustPayments_Helper::instance()->try_to_parse_date( $date_of_birth_string );
			if ( false !== $date_of_birth ) {
				$address->setDateOfBirth( $date_of_birth );
		...
		$gender_string = $customer->get_meta( '_trustpayments_shipping_gender', true, 'edit' );
		if ( ! empty( $gender_string ) ) {
			if ( strtolower( $gender_string ) == 'm' || strtolower( $gender_string ) == 'male' ) {
				$address->setGender( \TrustPayments\Sdk\Model\Gender::MALE );
			} elseif ( strtolower( $gender_string ) == 'f' || strtolower( $gender_string ) == 'female' ) {
		...
		return apply_filters( 'wc_trustpayments_modify_session_shipping_address', $address );
	}
	 ...
	 * @param \TrustPayments\Sdk\Model\Transaction $transaction transaction.
	 */
	protected function store_transaction_ids_in_session( \TrustPayments\Sdk\Model\Transaction $transaction ) {
		$session_handler = WC()->session;
		$session_handler->set( 'trustpayments_transaction_id', $transaction->getId() );
		$session_handler->set( 'trustpayments_space_id', $transaction->getLinkedSpaceId() );
	}
}
