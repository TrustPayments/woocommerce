...
/**
 *
 * WC_TrustPayments_Service_Refund Class
 *
 * TrustPayments
 ...
 */
class WC_TrustPayments_Service_Refund extends WC_TrustPayments_Service_Abstract {

	/**
	 * The refund API service.
	 *
	 * @var \TrustPayments\Sdk\Service\RefundService refund service.
	 */
	...
	public function create( WC_Order $order, WC_Order_Refund $refund ) {
		...
		$transaction = WC_TrustPayments_Service_Transaction::instance()->get_transaction( $transaction_info->get_space_id(), $transaction_info->get_transaction_id() );
	 ...
	 * @param \TrustPayments\Sdk\Model\LineItemReductionCreate[] $reductions reductions.
	 * @return \TrustPayments\Sdk\Model\LineItemReductionCreate[]
	 * @throws Exception Exception.
	 */
	protected function fix_reductions( WC_Order_Refund $refund, \TrustPayments\Sdk\Model\Transaction $transaction, array $reductions ) {
	 ...
	 * @param \TrustPayments\Sdk\Model\LineItem[]                $base_line_items base line items.
	...
	private function distribute_rounding_difference( array $reductions, $index, $remainder, array $base_line_items,
		...
		if ( 0 != $new_remainder && $index + 1 < count( $reductions ) ) {
			...
			if ( $new_remainder > pow( 0.1, $digits + 1 ) ) {
				throw new Exception( __( 'Could not distribute the rounding difference.', 'woo-trustpayments' ) );
			} else {
	...
	protected function get_reductions( WC_Order $order, WC_Order_Refund $refund ) {
		...
		foreach ( $refund->get_items() as $item ) {
			...
			$unique_id = $order_item->get_meta( '_trustpayments_unique_line_item_id', true );
		...
		foreach ( $refund->get_fees() as $fee ) {
			...
			$unique_id = $order_fee->get_meta( '_trustpayments_unique_line_item_id', true );
		...
		foreach ( $refund->get_shipping_methods() as $shipping ) {
			...
			$unique_id = $order_shipping->get_meta( '_trustpayments_unique_line_item_id', true );
	 ...
	 *
	 * @param int                                           $space_id space id.
	 * @param \TrustPayments\Sdk\Model\RefundCreate $refund refund.
	 * @return \TrustPayments\Sdk\Model\Refund
	 * @throws Exception Exception.
	 */
	public function refund( $space_id, \TrustPayments\Sdk\Model\RefundCreate $refund ) {
	 ...
	 *
	 * @param \TrustPayments\Sdk\Model\Transaction $transaction transaction.
	 * @param \TrustPayments\Sdk\Model\Refund      $refund refund.
	 * @return \TrustPayments\Sdk\Model\LineItem[]
	 * @throws Exception Exception.
	 */
	protected function get_base_line_items( \TrustPayments\Sdk\Model\Transaction $transaction, \TrustPayments\Sdk\Model\Refund $refund = null ) {
	 ...
	 *
	 * @param \TrustPayments\Sdk\Model\Transaction $transaction transaction.
	 * @return \TrustPayments\Sdk\Model\TransactionInvoice transaction invoice.
	 * @throws \TrustPayments\Sdk\ApiException ApiException.
	 */
	protected function get_transaction_invoice( \TrustPayments\Sdk\Model\Transaction $transaction ) {
		$query = new \TrustPayments\Sdk\Model\EntityQuery();
		...
		$invoice_service = new \TrustPayments\Sdk\Service\TransactionInvoiceService( WC_TrustPayments_Helper::instance()->get_api_client() );
	 ...
	 *
	 * @param \TrustPayments\Sdk\Model\Transaction $transaction transaction.
	 * @param \TrustPayments\Sdk\Model\Refund|null $refund refund.
	 * @return false|\TrustPayments\Sdk\Model\Refund
	 * @throws \TrustPayments\Sdk\ApiException ApiException.
	 */
	protected function get_last_successful_refund( \TrustPayments\Sdk\Model\Transaction $transaction, \TrustPayments\Sdk\Model\Refund $refund = null ) {
		$query = new \TrustPayments\Sdk\Model\EntityQuery();

		$filter = new \TrustPayments\Sdk\Model\EntityQueryFilter();
		$filter->setType( \TrustPayments\Sdk\Model\EntityQueryFilterType::_AND );
		$filters = array(
			$this->create_entity_filter( 'state', \TrustPayments\Sdk\Model\RefundState::SUCCESSFUL ),
			$this->create_entity_filter( 'transaction.id', $transaction->getId() ),
		);
		if ( null != $refund ) {
			$filters[] = $this->create_entity_filter( 'id', $refund->getId(), \TrustPayments\Sdk\Model\CriteriaOperator::NOT_EQUALS );
		}
		...
		$query->setOrderBys(
			array(
				$this->create_entity_order_by( 'createdOn', \TrustPayments\Sdk\Model\EntityQueryOrderByType::DESC ),
			)
		);
	...
	protected function get_refund_service() {
		if ( null == $this->refund_service ) {
			$this->refund_service = new \TrustPayments\Sdk\Service\RefundService( WC_TrustPayments_Helper::instance()->get_api_client() );
		}
