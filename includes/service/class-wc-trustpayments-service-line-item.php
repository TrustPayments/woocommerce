...
/**
 *
 * WC_TrustPayments_Service_Line_Item Class
 *
 * TrustPayments
 ...
 */
class WC_TrustPayments_Service_Line_Item extends WC_TrustPayments_Service_Abstract {
	...
	public function get_items_from_session() {
		...
		return WC_TrustPayments_Helper::instance()->cleanup_line_items( $combined, $cart->total, $currency );
	}
	...
	protected function create_product_line_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_cart() as $values ) {
			...
			if (isset($values['key'])) {
				$coupon_discount = apply_filters( 'wc_trustpayments_packages_coupon_percentage_discounts_by_item', $values['key'] );
			...
			$items[] = apply_filters( 'wc_trustpayments_modify_line_item_product_session', $this->clean_line_item( $line_item ), $values );
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_session( WC_Cart $cart, $currency ) {
		...
		foreach ( $cart->get_fees() as $fee ) {
			...
			if ( $amount_including_tax < 0 ) {
				...
				$line_item->setType( \TrustPayments\Sdk\Model\LineItemType::DISCOUNT );
				$line_item->setUniqueId( 'discount-' . $fee->id );
			} else {
				$line_item->setType( \TrustPayments\Sdk\Model\LineItemType::FEE );
				$line_item->setUniqueId( 'fee-' . $fee->id );
			}
			$fees[] = apply_filters( 'wc_trustpayments_modify_line_item_fee_session', $this->clean_line_item( $line_item ), $fee );
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_session( $packages, $chosen_methods, $currency ) {
		...
		foreach ( $packages as $package_key => $package ) {
			if ( isset( $chosen_methods[ $package_key ], $package['rates'][ $chosen_methods[ $package_key ] ] ) ) {
				...
				$line_item->setType( \TrustPayments\Sdk\Model\LineItemType::SHIPPING );
				$line_item->setUniqueId( WC_TrustPayments_Unique_Id::get_uuid() );

				$shippings[] = apply_filters( 'wc_trustpayments_modify_line_item_shipping_session', $this->clean_line_item( $line_item ), $shipping_rate );
			}
		}
		return $shippings;
	}
	...
	private function create_coupon_line_item( $coupon, float $amount = 0 ) {
		...
		$line_item->setName( sprintf( '%s: %s', WC_TrustPayments_Packages_Coupon_Discount::COUPON, $coupon->get_code() ) );
		$line_item->setQuantity( 1 );
		$line_item->setShippingRequired( false );
		$line_item->setSku( $sku, 200 );
		$line_item->setType( \TrustPayments\Sdk\Model\LineItemType::DISCOUNT );
	...
	public function get_items_from_order( WC_Order $order ) {
		...
		$items = apply_filters( 'wc_trustpayments_modify_line_item_order', $raw, $order );
		$total = apply_filters( 'wc_trustpayments_modify_total_to_check_order', $order->get_total(), $order );
		return WC_TrustPayments_Helper::instance()->cleanup_line_items( $items, $total, $order->get_currency() );
	}
	...
	protected function create_product_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order_items as $item ) {
			...
			$coupon_discount = $item->get_meta( '_trustpayments_coupon_discount_line_item_discounts' );
			if ( $coupon_discount > 0 ) {
				//there is some coupon discount applied
				$amount = $amount_including_tax;
			...
			$line_item->setType( \TrustPayments\Sdk\Model\LineItemType::PRODUCT );
			$line_item->setUniqueId( $item->get_meta( '_trustpayments_unique_line_item_id', true ) );
			...
			$items[] = apply_filters( 'wc_trustpayments_modify_line_item_product_order', $this->clean_line_item( $line_item ), $item );
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee ) {
			...
			$line_item->setUniqueId( $fee->get_meta( '_trustpayments_unique_line_item_id', true ) );

			$fees[] = apply_filters( 'wc_trustpayments_modify_line_item_fee_order', $this->clean_line_item( $line_item ), $fee );
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_order( WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping ) {
			...
			$line_item->setType( \TrustPayments\Sdk\Model\LineItemType::SHIPPING );
			$line_item->setUniqueId( $shipping->get_meta( '_trustpayments_unique_line_item_id', true ) );

			$shippings[] = apply_filters( 'wc_trustpayments_modify_line_item_shipping_order', $this->clean_line_item( $line_item ), $shipping );
		}
		return $shippings;
	}
	...
	public function get_items_from_backend( array $backend_items, $amount, WC_Order $order ) {
		...
		return WC_TrustPayments_Helper::instance()->cleanup_line_items( $combined, $amount, $order->get_currency() );
	}
	...
	protected function create_product_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$items[] = apply_filters( 'wc_trustpayments_modify_line_item_product_backend', $this->clean_line_item( $line_item ), $item );
		}
		return $items;
	}
	...
	protected function create_fee_lines_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_fees() as $fee_id => $fee ) {
			...
			$fees[] = apply_filters( 'wc_trustpayments_modify_line_item_fee_backend', $this->clean_line_item( $line_item ), $fee );
		}
		return $fees;
	}
	...
	protected function create_shipping_line_items_from_backend( array $backend_items, WC_Order $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping_id => $shipping ) {
			...
			$shippings[] = apply_filters( 'wc_trustpayments_modify_line_item_shipping_backend', $this->clean_line_item( $line_item ), $shipping );
		}
		return $shippings;
	}
	 ...
	 *
	 * @param \TrustPayments\Sdk\Model\LineItemCreate $line_item line item.
	 * @return \TrustPayments\Sdk\Model\LineItemCreate
	 */
	protected function clean_line_item( \TrustPayments\Sdk\Model\LineItemCreate $line_item ) {
	...
	protected function get_taxes( array $rates_or_ids ) {
		...
		foreach ( array_keys( $rates_or_ids ) as $rate_id ) {
			$tax_rates[] = new \TrustPayments\Sdk\Model\TaxCreate(
				array(
	...
	private function get_base_attributes( $product_id ) {
		...
		foreach ( $products_to_check as $id ) {
			...
			if ( $wpm_gtin_code ) {
				$wpm_gtin_attribute = new \TrustPayments\Sdk\Model\LineItemAttributeCreate();
			...
			foreach ( $product_attributes as $key => $object ) {
				if ( is_a( $object, 'WC_Product_Attribute' ) ) {
					if ( $object->is_taxonomy() ) {
						$attribute_options = WC_TrustPayments_Entity_Attribute_Options::load_by_attribute_id( $object->get_id() );
