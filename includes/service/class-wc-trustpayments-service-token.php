 ...
 */
class WC_TrustPayments_Service_Token extends WC_TrustPayments_Service_Abstract {
	...
	public function update_token($space_id, $token_id){
		...
		$filter->setChildren(
				array(
				    ...
				    $this->create_entity_filter('state', \TrustPayments\Sdk\Model\TokenVersionState::ACTIVE) 
				));
		...
		if (!empty($token_versions)) {
		    ...
		    $info = WC_TrustPayments_Entity_Token_Info::load_by_token($space_id, $token_id);
			if ($info->get_id()) {
				$info->delete();
			}
		}
	...
	protected function update_info($space_id, \TrustPayments\Sdk\Model\TokenVersion $token_version){
		/* @var WC_TrustPayments_Entity_Token_Info $info */
	    $info = WC_TrustPayments_Entity_Token_Info::load_by_token($space_id, $token_version->getToken()->getId());
		...
		/* @var WC_TrustPayments_Entity_Method_Configuration $paymentMethod */
		
		$payment_method = WC_TrustPayments_Entity_Method_Configuration::load_by_configuration($space_id, 
	...
	protected function get_token_service(){
		if ($this->token_service == null) {
		    $this->token_service = new \TrustPayments\Sdk\Service\TokenService(WC_TrustPayments_Helper::instance()->get_api_client());
		}
	...
	protected function get_token_version_service(){
		if ($this->token_version_service == null) {
		    $this->token_version_service = new \TrustPayments\Sdk\Service\TokenVersionService(WC_TrustPayments_Helper::instance()->get_api_client());
		}
