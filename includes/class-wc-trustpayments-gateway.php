...
class WC_TrustPayments_Gateway extends WC_Payment_Gateway {
	...
	public function get_description(){
		$description = '';
		if ($this->tru_show_description == 'yes') {
	...
	public function init_form_fields(){
		$this->form_fields = array(
			...
			'description_value' => array(
				...
				'type' => 'info',
				'value' => !empty($this->get_description()) ? $this->get_description() : __('[not set]', 'woo-trustpayments'),
...
<?php
	...
	public function is_available(){
		...
		if(apply_filters('wc_trustpayments_is_order_pay_endpoint', is_checkout_pay_page())){
		    ...
		    try {
				...
				$isClientError = ($response_object instanceof \TrustPayments\Sdk\Model\ClientError);
				if ($isClientError) {
	...
	public function payment_fields(){
	    ...
	    try{
            ...
            if (!wp_script_is( 'trustpayments-remote-checkout-js', 'enqueued' )) {
    	        ...
    	        wp_enqueue_script('trustpayments-remote-checkout-js', $ajax_url, array(
    	            'jquery'
    	        ), null, true);
    	        wp_enqueue_script('trustpayments-checkout-js', WooCommerce_TrustPayments::instance()->plugin_url() . '/assets/js/frontend/checkout.js',
    	            array(
    	                'jquery',
            ...
            <div id="payment-form-<?php echo $this->id?>">
            	<input type="hidden" id="trustpayments-iframe-possible-<?php echo $this->id?>" name="trustpayments-iframe-possible-<?php echo $this->id?>" value="false" />
            </div>
            <input type="hidden" id="trustpayments-space-id-<?php echo $this->id?>" name="trustpayments-space-id-<?php echo $this->id?>" value="<?php echo $transaction->getLinkedSpaceId(); ?>"  />
        	<input type="hidden" id="trustpayments-transaction-id-<?php echo $this->id?>" name="trustpayments-transaction-id-<?php echo $this->id?>" value="<?php echo $transaction->getId(); ?>"  />
        	<input type="hidden" id="trustpayments-transaction-nonce-<?php echo $this->id?>" name="trustpayments-transaction-nonce-<?php echo $this->id?>" value="<?php echo $transaction_nonce; ?>" />
            <div id="trustpayments-method-configuration-<?php echo $this->id?>"
	...
	public function process_payment($order_id){
	    ...
	    $space_id = $_POST['trustpayments-space-id-'.$this->id];
	    $transaction_id = $_POST['trustpayments-transaction-id-'.$this->id];
	    $transaction_nonce = $_POST['trustpayments-transaction-nonce-'.$this->id];
	    ...
	    $no_iframe = isset($_POST['trustpayments-iframe-possible-'.$this->id]) && $_POST['trustpayments-iframe-possible-'.$this->id] == 'false';
