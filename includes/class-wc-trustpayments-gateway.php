...
<tr valign="top">
	...
	public function is_available() {
		...
		if (is_checkout() && isset($wp->query_vars['order-received'])) {
			...
			if (!empty($gateway_available[$this->tru_payment_method_configuration_id])) {
				return $gateway_available[$this->tru_payment_method_configuration_id];
			}
			else {
				return false;
			}
		...
		$gateway_available = WC()->session->get('whitelabel_payment_gateways');
		$gateway_available[$this->tru_payment_method_configuration_id] = true;
