...
/**
 *
 * WC_TrustPayments_Customer_Document Class
 *
 * TrustPayments
...
class WC_TrustPayments_Customer_Document {
	...
	public static function render_download_buttons( $order_id ) {
		...
		$packing = false;
		$invoice = false;
		if ( get_option( WooCommerce_TrustPayments::CK_CUSTOMER_INVOICE ) == 'yes' && in_array(
...
<section class="woocommerce-order-trustpayments-documents">
	<h2><?php esc_html_e( 'Order Documents', 'woo-trustpayments' ); ?></h2>
				 <?php if ( $invoice ) : ?>
					<span><a
		href="
						<?php
						...
						?>
				"
		class="woocommerce-order-trustpayments-download woocommerce-order-trustpayments-download-invoice"><?php esc_html_e( 'Download Invoice', 'woo-trustpayments' ); ?></a></span>
				<?php endif; ?>
				<?php if ( $packing ) : ?>
				<span><a
		href="
					...
					?>
				"
		class="woocommerce-order-trustpayments-download woocommerce-order-trustpayments-download-packingslip"><?php esc_html_e( 'Download Packing Slip', 'woo-trustpayments' ); ?></a></span>
				<?php endif; ?>
	...
	public static function download_document() {
		...
		// verify nonce.
		$action = isset( $_GET['trustpayments_action'] ) ? sanitize_key( $_GET['trustpayments_action'] ) : false;
