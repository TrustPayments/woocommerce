...
/**
 * WC_TrustPayments_Packages_Coupon_Discount Class
 *
 * TrustPayments
...
/**
 * Class WC_TrustPayments_Packages_Coupon_Discount.
 * 
...
class WC_TrustPayments_Packages_Coupon_Discount {
	...
	public static function init() {
		...
		add_filter(
			'wc_trustpayments_packages_coupon_discount_totals_including_tax',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_trustpayments_packages_coupon_cart_has_coupon_discounts_applied',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_trustpayments_packages_coupon_discounts_applied_by_cart',
			array(
				__CLASS__,
		...
		add_filter(
			'wc_trustpayments_packages_coupon_percentage_discounts_by_item',
			array(
				__CLASS__,
	...
	public static function copy_total_coupon_discount_meta_data_to_order_item( WC_Order_Item_Product $item, $cart_item_key, $values, WC_Order $order = null ) {
		$coupon_discount = apply_filters('wc_trustpayments_packages_coupon_percentage_discounts_by_item', $cart_item_key );
		$item->add_meta_data( '_trustpayments_coupon_discount_line_item_discounts', $coupon_discount );
		return $item;
	}
	...
	public static function get_coupons_discount_totals_including_tax( $currency ) {
		...
		foreach (WC()->cart->get_coupon_discount_totals() as $coupon_discount_total) {
			$coupons_discount_total += WC_TrustPayments_Helper::instance()->round_amount( $coupon_discount_total, $currency );
		}
	...
	public static function has_cart_coupon_discounts_applied( $currency ) {
		$discount = apply_filters( 'wc_trustpayments_packages_coupon_discount_totals_including_tax', $currency );
		return $discount > 0;
	}
	...
	public static function get_coupon_percentage_discount_by_item( string $item_key ) {
		$coupon_discounts_applied = apply_filters( 'wc_trustpayments_packages_coupon_discounts_applied_by_cart', WC()->cart );
...
WC_TrustPayments_Packages_Coupon_Discount::init();
