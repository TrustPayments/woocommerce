...
jQuery(function($) {
    ...
    var wc_trustpayments_management = {

	init : function() {
	    this.handle_refund_button();
	    this.show_refund_states();
	    $('#woocommerce-order-items').off('click.woo-trustpayments');
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.trustpayments-completion-show', {
			self : this
		    }, this.show_completion);
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.action-trustpayments-completion-cancel', {
			self : this
		    }, this.cancel_completion);
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.action-trustpayments-completion-execute', {
			self : this
		    }, this.execute_completion);
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.trustpayments-void-show', {
			self : this
		    }, this.show_void);
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.action-trustpayments-void-cancel', {
			self : this
		    }, this.cancel_void);
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.action-trustpayments-void-execute', {
			self : this
		    }, this.execute_void);
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.trustpayments-update-order', {
			self : this
		    }, this.update_order);
	    $('#woocommerce-order-items').on('click.woo-trustpayments',
		    'button.refund-items', {
	...
	handle_refund_button : function() {
	    if ($('span#trustpayments-remove-refund').length > 0) {
	...
	show_refund_states : function() {
	    $('tbody#order_refunds div.trustpayments-refund-status')
		    .each(
			    function() {
				...
				if (state == 'failure') {
				    refund_thumb
					    .addClass('trustpayments-refund-state-failue');
				} else if (state == 'pending'
					|| state == 'sent'
					|| state == 'created') {
				    refund_thumb
					    .addClass('trustpayments-refund-state-pending');
				}
			    });
	...
	show_completion : function(event) {
	    ...
	    $('div.wc-order-add-item').children(
		    'button.trustpayments-completion-button').each(
		    function(key, value) {
			$(value).show();
	    ...
	    $('div.wc-order-refund-items').find(
		    '#completion_restock_not_completed_items').show();
	    $('div.wc-order-refund-items').find(
		    'label[for="completion_restock_not_completed_items"]')
		    .show();
	    $('div.wc-order-refund-items').find(
		    '#completion_restock_not_completed_items').closest('tr')
		    .show();
	    ...
	    $('#woocommerce-order-items').find('input.line_total').each(
			...
			$(this).closest('td.line_cost').find(
				'input.refund_line_total').val($(this).val());

			$(this).closest('td.line_cost').find(
				'input.refund_line_total').data(
				'trustpayments-initial-amount', $(this).val());
	    ...
	    $('#woocommerce-order-items').find('input.line_tax').each(
			...
			$(this).closest('td.line_tax').find(
				'input.refund_line_tax').val($(this).val());
			$(this).closest('td.line_tax').find(
				'input.refund_line_tax').data(
				'trustpayments-initial-tax', $(this).val());
		    });
	...
	update_taxes_for_line_items : function() {
	    var initial_amount = $(this).data('trustpayments-initial-amount');
	...
	execute_completion : function() {
	    ...
	    if (window.confirm(trustpayments_admin_js_params.i18n_do_completion)) {
			...
			.each(
				function(index, item) {
					...
					line_item_tax_totals[$(item).closest(
						'tr').data('order_item_id')][tax_id] = accounting
						.unformat(
							item.value,
							woocommerce_admin.mon_decimal_point);
				    }
		...
		var data = {
		    action : 'woocommerce_trustpayments_execute_completion',
		    order_id : woocommerce_admin_meta_boxes.post_id,
		    completion_amount : refund_amount,
		    line_item_qtys : JSON.stringify(line_item_qtys, null, ''),
		    line_item_totals : JSON.stringify(line_item_totals, null,
			    ''),
		    line_item_tax_totals : JSON.stringify(line_item_tax_totals,
			    null, ''),
	...
	show_void : function() {
	    ...
	    $('div.wc-order-add-item').children('button.trustpayments-void-button')
	...
	execute_void : function() {
	    ...
	    if (window.confirm(trustpayments_admin_js_params.i18n_do_void)) {

		var data = {
		    action : 'woocommerce_trustpayments_execute_void',
		    order_id : woocommerce_admin_meta_boxes.post_id,
	...
	update_order : function() {
	    ...
	    var data = {
			action : 'woocommerce_trustpayments_update_order',
			order_id : woocommerce_admin_meta_boxes.post_id,
			security : woocommerce_admin_meta_boxes.order_item_nonce
	    };
    ...
    wc_trustpayments_management.init();
