...
/* global jQuery document window trustpayments_js_params */

jQuery(
	function ($) {
		var wc_trustpayments_checkout = {
			payment_methods : {},
			integrations: {
				LIGHTBOX: 'lightbox',
				IFRAME: 'iframe'
			 ...
			 */
			info : function() {
				var versions = window.trustpayments_js_params.versions || {};
				var info = [
					{library: 'integration', version: window.trustpayments_js_params.integration},
					...
					{library: 'woo-trustpayments', version: versions.trustpayments || null},
				];
				console.table(info, []);
			},
			...
			init : function() {
				// Payment methods.
				this.$checkout_form.off( 'click.woo-trustpayments' ).on(
					'click.woo-trustpayments',
				...
				this.$checkout_form.off( 'button#place_order' ).on(
					...
					{ self : this },
					function (event) {
						if (trustpayments_js_params.integration === window.wc_trustpayments_checkout.integrations.LIGHTBOX ) {
							return false;
						}
					}
				);
				...
				if ($( document.body ).hasClass( 'woocommerce-order-pay' )) {
					...
					this.$order_review.off( 'click.woo-trustpayments' ).on(
						'click.woo-trustpayments',
						'input[name="payment_method"]',
						{
							self : this
				...
				window.wc_trustpayments_checkout = this;
			},
			...
			payment_method_click : function(event) {
				...
				var configuration = $( "#trustpayments-method-configuration-" + current_method );
			...
			enable_place_order_button: function(){
				...
				this.$order_button.removeClass( 'trustpayments-disabled' );
			},

			disable_place_order_button: function(){
				this.$order_button.prop( 'disabled', true );
				this.$order_button.addClass( 'trustpayments-disabled' );
			},
			...
			is_supported_method : function(method_id) {
				return method_id && (method_id.indexOf( 'trustpayments_' ) === 0);
			},
			...
			register_method : function(method_id, configuration_id, container_id) {
				...
				if (this.checkout_form_identifier === '#order_review') {
					this.$checkout_form.off( 'submit.trustpayments' )
					.on(
						'submit.trustpayments',
						function(){
					...
					this.$checkout_form.off( 'checkout_place_order_' + method_id + '.trustpayments' )
					.on(
						'checkout_place_order_' + method_id + '.trustpayments',
						function(){
							return self.process_submit( method_id );
						}
					);
			...
			process_submit : function(method_id){
				...
				if ( ! has_full_address) {
					...
					this.submit_error( trustpayments_js_params.i18n_not_complete );
					return false;
				}
						...
						$.ajax(
							{
								...
								success: function( result ) {
									...
									if (typeof result.trustpayments == 'undefined') {
			...
			process_order_created : function(data, textStatus, jqXHR) {
				...
				if (trustpayments_js_params.integration && trustpayments_js_params.integration === self.integrations.LIGHTBOX ) {
					...
					if ( ! has_full_address) {
						...
						self.submit_error( trustpayments_js_params.i18n_not_complete );
						return false;
					}
					...
					if (typeof data.trustpayments == 'undefined') {
						return false;
					}
