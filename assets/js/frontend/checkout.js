...
jQuery(function ($) {
    var wc_trustpayments_checkout = {
        ...
        init : function() {
            // Payment methods
            $('form.checkout').off('click.woo-trustpayments').on(
            ...
            if ($(document.body).hasClass('woocommerce-order-pay')) {
                ...
                $('#order_review').off('click.woo-trustpayments').on('click.woo-trustpayments',
                    'input[name="payment_method"]', {
                        self : this
                    }, this.payment_method_click);
            }
        ...
        enable_place_order_button: function(){
            ...
            order_button.removeClass('trustpayments-disabled');
        },
        
        disable_place_order_button: function(){
            var order_button = $('#place_order');
            order_button.prop('disabled', true);
            order_button.addClass('trustpayments-disabled');
        },
        ...
        register_method : function(method_id, configuration_id, container_id) {
            ...
            if(this.checkout_form_identifier === '#order_review'){
                $(this.checkout_form_identifier).off('submit.trustpayments').on(
                ...
                form.off('checkout_place_order_' + method_id + '.trustpayments')
                    .on('checkout_place_order_' + method_id + '.trustpayments', function(){
                        return self.process_submit(method_id);
                    });
            }
        },
